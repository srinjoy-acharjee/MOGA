...
public class MOGA_ANN 
{
    ...
    Population_of_ANN pop_ANN,acc_GA;
    ...
    MOGA_ANN(Dataset training_DataSet,Dataset testing_DataSet,int population_size,float crossover_probability,float mutation_probability,int number_of_generation,int initial_hidden_nodes)
    {
        ...
        for(int generation=0;generation<number_of_generation;generation++)
        {
          ...
          //System.out.println("combine population aftre remove unnecessary weight.................................................................");
            Population combination_population_after_remove_unnecessary_weight=Removing_unnecessary_weight(training_DataSet,combination_population,initial_hidden_nodes);
            //combination_population_after_remove_unnecessary_weight.Show_population();
            ...
            //System.out.println("after bpn population");
             ...
             acc_GA = new Population_of_ANN(combination_population_after_remove_unnecessary_weight,training_DataSet,testing_DataSet, initial_hidden_nodes);
             
             
             pop_ANN=new Population_of_ANN(combination_population_after_remove_unnecessary_weight,training_DataSet,testing_DataSet,initial_hidden_nodes);
            //System.out.println("after bpn population");
            //combination_population.Show_population();
            //combination_population.max_fitness();
            //combination_population.max_zero();
            //System.out.println("entering unique population");
            Population unique_population=Unique_population(pareto_pop,combination_population_after_remove_unnecessary_weight,training_DataSet,initial_hidden_nodes);
         ...
         //System.out.println();
             ...
             // {
              ...
              //  }System.out.println();
                ...
                System.out.println("pareto \taccuracy="+pareto_pop.chromo[pareto_pop.fit_chromo].fitness+"\t"+"no of zero="+pareto_pop.chromo[pareto_pop.max_zero_chromo].num_of_zero);
                //pareto_pop.Show_population();
          System.out.println();
    ...
    public Population Crossover(Dataset training_DataSet,Population pop,float crossover_probability,int initial_hidden_nodes)
    {
        ...
        for (int counter=0;counter<=number_of_crossover-1;counter++)
        {
            ...
            int random3=(int)(Math.random()*pop.chromo[random1].gene.length);//random3 is the crossover point
            //System.out.println(random3);//for testing
            Chromosome child1[]=new Chromosome[pop.chromo[random1].gene.length];
            Chromosome child2[]=new Chromosome[pop.chromo[random2].gene.length];
            for (int loop_counter=0;loop_counter<=pop.chromo[random1].gene.length-1;loop_counter++)
            {
                if(loop_counter<=random3)
                    pop_after_crossover.chromo[random1].gene[loop_counter].gene_value=pop.chromo[random1].gene[loop_counter].gene_value;
                else
                    pop_after_crossover.chromo[random2].gene[loop_counter].gene_value=pop.chromo[random1].gene[loop_counter].gene_value;
            }
            for (int loop_counter=0;loop_counter<=pop.chromo[random2].gene.length-1;loop_counter++)
            {
                if(loop_counter<=random3)
                    pop_after_crossover.chromo[random2].gene[loop_counter].gene_value=pop.chromo[random2].gene[loop_counter].gene_value;
                else
                    pop_after_crossover.chromo[random1].gene[loop_counter].gene_value=pop.chromo[random2].gene[loop_counter].gene_value;
            }
    ...
    public Population Mutation(Dataset training_DataSet,Population pop,float mutation_probability,int initial_hidden_nodes)
    {
        ...
        for(int counter=0;counter<pop_after_mutation.pop_size;counter++)
        {
            ...
            {
                float random=(float) ((Math.round((float)Math.random()*10))/10.0);
                //System.out.println(random);
                if(random < mutation_probability)
                {
                    //System.out.println("chromo"+counter+"gene"+counter1);//for testing
                    pop_after_mutation.chromo[counter].gene[counter1].gene_value=(Math.round(((double)(((Math.random()*2.0)-1.0)*0.1))*100))/100.00;
                }
            }
        }
    ...
    /********************************************************************************************************
     *       This method is to remove unnecessary weights range(-0.01 to  -0.01)                            *
     *       and update the chromosome weight                                                               *
     ********************************************************************************************************/ 
   public Population Removing_unnecessary_weight(Dataset training_DataSet,Population combination_population,int initial_hidden_nodes)
    {
	   int combination_population_size=combination_population.pop_size;
	   Population combination_population_afrter_remove_unnecessary_weight=new Population(training_DataSet,combination_population_size,initial_hidden_nodes);
	   for(int counter=0;counter<combination_population.pop_size;counter++)
       {
         ...
         {
       		 if(combination_population.chromo[counter].gene[counter1].gene_value >=-0.01 && combination_population.chromo[counter].gene[counter1].gene_value<=0.01)
                     {
       		    	     //System.out.print("["+counter+"]["+counter1+"]"); 
       		    	     combination_population_afrter_remove_unnecessary_weight.chromo[counter].gene[counter1].gene_value=0.00;
                     }
                  else
                     {
                      	 combination_population_afrter_remove_unnecessary_weight.chromo[counter].gene[counter1].gene_value=combination_population.chromo[counter].gene[counter1].gene_value;
                     }
            }//end inner for loop
       }//end for outer loop
	   System.out.println();
	  // population_after_bpn=new Population(training_DataSet,combination_population_afrter_remove_unnecessary_weight.pop_size,initial_hidden_nodes);
       //population_after_bpn=combination_population_afrter_remove_unnecessary_weight;
	   return combination_population_afrter_remove_unnecessary_weight;
   }//end of the method
    ...
    {
        int flag_size=0;
        //Population unique_population=new Population(training_DataSet,combination_population_size,initial_hidden_nodes);
        ...
        for(int counter=0;counter< combination_population.pop_size-1;counter++)
        {
            ...
            {
                if((combination_population.chromo[counter].toString()).equals(combination_population.chromo[counter1].toString()))
                {
                   flag[counter1]=false;
